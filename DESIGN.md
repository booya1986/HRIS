...
## Illustrations
Style: hand-drawn doodles with **no background** (transparent PNG, the page's white shows through; the OG cover is on plain white), black marker line art, simple black blob characters with white eyes, small icons (clock, tangle, warning sign), with the accent in **red** (`#e5322d`) instead of green. Short English labels only (image models render Hebrew unreliably). Generated with `gpt_image_2_5` via Higgsfield with `background: transparent` (opaque for the OG cover); the style reference is `assets/style-reference.png`. Every illustration gets an `alt` in Hebrew and sits full-width inside the 46rem measure with a muted caption.

Current set (`assets/`): `og.png` (1200x630 cover), `stages.png` (three stages staircase, section 5), `payroll-anchor.png` (global core, interface, local payroll, section 3), `treadmill.png` (no launch, maintenance forever, section 7), `headcount.png` (HR vs Finance headcount and the data dictionary, section 4), `queue.png` (protect the team from the queue, section 9.2). Placement and captions live in `components.ILL`; a figure is inserted right after the heading whose text contains the needle.
